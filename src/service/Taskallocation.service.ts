...
export class TaskallocationService {
    ...
    async createTaskAllocation(taskallocationDTO: TaskallocationDTO): Promise<Taskallocation001wb[]> {
        ...
        for (let i = 0; i < sheet.length; i++) {
            ...
            console.log(" taskallocation001wb.insertDatetime", taskallocation001wb.insertDatetime);
    ...
    async findByCuratorStartEndDate(username: any, startDate: any, endDate: any): Promise<Taskallocation001wb[]> {
        ...
        let task = await this.taskAllocateRepository.find({ where: { curatorName: username, updatedDatetime: Between(sDate, eDate), status: "Submitted to QC" } });
        return task;
    }
    ...
    async adminStartEndDateExportExcel(startDate: any, endDate: any, @Req() request: Request, @Res() response: Response): Promise<Taskallocation001wb[]> {
        // if (startDate != endDate) {
        let sDate = new Date(startDate);
        sDate.setDate(sDate.getDate() - 1);
        let eDate = new Date(endDate);
        ...
        let task = await this.taskAllocateRepository.find({ where: { insertDatetime: Between(sDate, eDate) } });

        if(task.length == 0){
            throw new HttpException('This Date have no Data!', HttpStatus.NOT_FOUND);
       }else{
        return await ReportTaskAllocation(task, request, response);
       }
       
    }

    async adminBatchNumberExportExcel(batchNo: any, request: any, response: any): Promise<Taskallocation001wb[]> {

        let task = await this.taskAllocateRepository.find({ where: { cbatchNo: batchNo } });
        if(task.length == 0){
            throw new HttpException('This batchNo have no Data!', HttpStatus.NOT_FOUND);
       }else{
        return await ReportTaskAllocation(task, request, response);
       }
    ...
    async curatorNameExportExcel(curatorName: any, request: any, response: any): Promise<Taskallocation001wb[]> {

        let task = await this.taskAllocateRepository.find({ where: { curatorName: curatorName } });
        
        if(task.length == 0){
            throw new HttpException('This curatorName have no Data!', HttpStatus.NOT_FOUND);
       }else{
        return await ReportTaskAllocation(task, request, response);
       }
    ...
    async reviewerNameExportExcel(reviewerName: any, request: any, response: any): Promise<Taskallocation001wb[]> {

        let task = await this.taskAllocateRepository.find({ where: { reviewerName: reviewerName } });
        if(task.length == 0){
            throw new HttpException('This reviewerName have no Data!', HttpStatus.NOT_FOUND);
       }else{
        return await ReportTaskAllocation(task, request, response);
       }
    ...
    async downloadExcel(@Req() request: Request, @Res() response: Response) {
        let task = await this.taskAllocateRepository.find();
        // await ReportTaskAllocation(task, request, response);
    }
}
async function ReportTaskAllocation(task, request, response) {
    if (task.length < 0) {
        return;
    }
    else {
