        ...
        if (assays.length < 0) {
            ...
            for (let i = 0; i < assays.length; i++) {
                ...
                if (assays[tempTan].ligandSlno2.tanNumber == assays[i].ligandSlno2.tanNumber) {
                    ...
                    tanNumber = assays[tempTan].ligandSlno2.tanNumber;
                    initRow++;
                    flag= true;
                    if(i == (assays.length-1)){
                        console.log("Testing-1--->",tanNumber);
                        await workbook.xlsx.writeFile('./EXCEL/export_'+tanNumber+'.xlsx');
                    }
                } else {
                    if(flag) {
                        console.log("Testing-2--->",tanNumber);
                        tanNumber = assays[tempTan].ligandSlno2.tanNumber;
                        await workbook.xlsx.writeFile('./EXCEL/export_'+tanNumber+'.xlsx');
                        initRow = 3;
                        workbook = new excel.Workbook();
                        worksheet = await ReportHeader(workbook);
                        flag= false;
                    } else if(i == (assays.length-1)){
                        console.log("Testing-3--->",tanNumber);
                        let assaycount=assays[i];
                        await ReportData(worksheet, initRow, assaycount, i);
                        initRow++;
                        tanNumber = assays[tempTan].ligandSlno2.tanNumber;
            ...
            // console.log("Testing-4--->",tanNumber);
            // workbook.xlsx.writeFile('./EXCEL/export_'+tanNumber+'.xlsx');
            ...
            // return workbook.xlsx.write(response, 'exportlast.xlsx').then(function () {
...
async function ReportData(worksheet, temp, assaycount, i){
    ...
    worksheet.getCell('AL' + temp).value = assaycount.dataLocator;
